...
def to_sparse_tensor(matrix: Union[sp.spmatrix, torch.Tensor],
    ...
    Returns:
        sparse_tensor: Resulting sparse tensor.
    """
    if device is None:
...
def coms_matrix_to_list(communities_matrix):
    ...
    nodes, communities = communities_matrix.nonzero() if sp.issparse(communities_matrix) else np.nonzero(communities_matrix)
...
def plot_sparse_clustered_adjacency(A, num_coms, z, o, ax=None, markersize=0.25):
    """Plot adjacency matrix reordered by community assignments.

    Args:
        A: Sparse adjacency matrix.
        num_coms: Number of communities.
        z: Community assignment per node (argmax of membership matrix), shape [N].
        o: Node ordering (e.g. np.argsort(z)), shape [N].
        ax: Matplotlib axes (uses current axes if None).
        markersize: Size of the dots in the spy plot.
    """
    ...
    ax.tick_params(axis='both', which='both', labelbottom=False, labelleft=False, labeltop=False)
...
def adjacency_split_naive(A, p_val, neg_mul=1, max_num_val=None):
    ...
    to_keep = np.asarray((1 - A[cne1, cne2])).astype(bool).ravel() * (cne1 != cne2)
